...
export default function ManufacturerLogin() {
  ...
  return (
    <AuthLayout
      ...
      subtitle="Access the clinical ledger to mint batches and track global provenance."
      ...
      <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="space-y-2">
            <div className="flex justify-between items-center">
                <label className="text-[11px] font-bold uppercase tracking-widest font-label text-on-surface-variant">Security Key</label>
                <a href="#" className="text-[11px] font-bold uppercase tracking-widest text-primary hover:text-primary-container">Lost key?</a>
            </div>
            <div className="relative">
                ...
                <Shield className="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant" size={20} />
            </div>
        </div>
        
        <button type="button" onClick={() => { login('manufacturer'); navigate('/manufacturer'); }} className="w-full bg-primary hover:brightness-110 active:scale-95 transition-all text-on-primary font-headline font-bold py-4 rounded-lg shadow-lg mt-6">
            Authenticate Node
        </button>
      </form>
      
      <p className="mt-8 text-[13px] text-center text-on-surface-variant font-body">
          Don't have a node yet? <br/><a href="/signup/manufacturer" className="text-secondary font-bold hover:underline">Apply for enterprise access</a>
      </p>
    </AuthLayout>
  );
}
